...
addEventHandler("onClientResourceStart", getResourceRootElement( getThisResource()), function()
	...
	if build<"04938" or ver < "1.3.1" then 
		--outputDebugString('Pixel Lighting: is not compatible with this client version!',3, 255, 0, 0)
		return
	end
	...
	if (shaderTable.smVersion~="3") then
		--outputDebugString('Pixel Lighting: shader model 3 not supported',3, 255, 0, 0)
	end
	shaderTable.isConfigChanged = true
	...
	if not shaderTable.isStarted  then 
		...
		--outputDebugString('Pixel Lighting: started',3, 0, 255, 0)			
	end
end
)
