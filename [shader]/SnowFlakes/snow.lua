...
function startSnow()
	...
	--	outputChatBox(string.format("Width/Depth: %.1f",box_width))
		addEventHandler("onClientRender",root,drawSnow, false, "high+99999")
		snowing = true
		return true
	else
...
function toggleSnow(on)
	if on == snowing then return false end
	if on then
		startSnow()
	else
		stopSnow() 
...
addEvent("switchSnowFlakes")
addEventHandler("switchSnowFlakes", root, function(on)
	toggleSnow(on)
end)
...
local snow = {}
-- default settings
settings = {type = "real", density = 200, wind_direction = {0.01,0.01}, wind_speed = 1, snowflake_min_size = 0.5, snowflake_max_size = 3, fall_speed_min = 1, fall_speed_max = 3, jitter = true}
