...
BEGIN
  ...
  DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;
	...
	-- Setting selection list
	UPDATE vrp_forum.wcf1_user_option SET selectOptions = (SELECT GROUP_CONCAT(a.Name SEPARATOR '\n') AS selectOptions FROM (SELECT wuo.defaultValue AS Name FROM vrp_forum.wcf1_user_option wuo WHERE wuo.optionID = 46 UNION SELECT f.Name FROM vrp_public.vrp_factions f) AS a) WHERE optionID = 46;
	-- Resetting values
