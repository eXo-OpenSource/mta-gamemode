...
function Forum:createAccount(player, username, password)
	fetchRemote(("http://www.v-roleplay.net/forum/wcf/lib/data/vrp/api/api.php?action=CreateAccount&username=%s&password=%s"):format(username, password), Async.waitFor(player))
	return Async.wait()
end