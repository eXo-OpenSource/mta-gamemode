...
function BankManager:Event_Transfer(toPlayerName, amount, reason)
	local client = client
	if tonumber(amount) and amount > 0 then
		...
		if toPlayerName == "San News" then
			client:transferBankMoney(CompanyManager:getSingleton():getFromId(CompanyStaticId.SANNEWS), amount, ("Spende an San News von %s\nZweck: %s"):format(client:getName(), reason), "Gameplay", "SanNewsDonation")
			client:triggerEvent("bankMoneyBalanceRetrieve", client:getBankMoney())
			CompanyManager:getSingleton():getFromId(CompanyStaticId.SANNEWS):addLog(client, "Spenden", ("hat %d$ gespendet! (Zweck: %s)"):format(amount, reason))
