...
function VehicleTexture:constructor(vehicle, path, textureName, type, force, isPreview)
	if vehicle and isElement(vehicle) then
		self.m_Id = #VehicleTexture.Map+1
		self.m_Type = type
		self.m_Vehicle = vehicle
		self.m_Path = path
		self.m_Texture = textureName
		if textureName then
			self.m_Texture = textureName
		...
		if force then
			local tab = {{vehicle = self.m_Vehicle, textureName = self.m_Texture, texturePath = self.m_Path, type = self.m_Type}}
			if not isPreview then
				VehicleTexture.sendToClient(getRootElement(), tab)
			else
				VehicleTexture.sendToClient(player, tab)
			end
		end
...
function VehicleTexture.requestTextures(target)
	...
	for index, instance in pairs(VehicleTexture.Map) do
		if instance.m_Vehicle and isElement(instance.m_Vehicle) then
			vehicleTab[#vehicleTab+1] = {vehicle = instance.m_Vehicle, textureName = instance.m_Texture, texturePath = instance.m_Path, type = instance.m_Type}
		end
	end
