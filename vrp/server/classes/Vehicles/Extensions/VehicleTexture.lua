...
function VehicleTexture:constructor(vehicle, path, texture, force, isPreview, previewClient)
		...
		if force then
			if self.m_Vehicle and isElement(self.m_Vehicle) then
				if not isPreview then
					...
					VehicleTexture.sendToClient(previewClient, {{vehicle = self.m_Vehicle, textureName = self.m_Texture, texturePath = self.m_Path, optional = self.m_Optional, isRequested = false}})
				end
			end
		end
