...
function WareStayTop:constructor( super )
	...
	for key, p in ipairs(self.m_Super.m_Players) do
		for i = 1, #self.m_RandomTiles do 
			...
			self.m_RotateTimer = setTimer(function() if self then p:triggerEvent("PlatformEnv:rotateTile", self.m_RandomTiles2[i], WareStayTop.rotateTimes[self.m_Super.m_Gamespeed]) end end,2000/self.m_Super.m_Gamespeed, 1)
		end
	end
end
