	...
	if button == "left" and state == "down" then
        if source:getModel() == self.m_Model then
            if player:getInventoryOld():getFreePlacesForItem("Kürbis") >= 1 then
				...
				if player:getInventoryOld():getItemAmount("Kürbis") >= 50 then
					player:giveAchievement(89) -- Kürbissammler
				end
            else
                player:sendError(_("Du kannst nicht soviele Kürbisse tragen! Maximal %d Stk.!", player, player:getInventoryOld():getMaxItemAmount("Kürbis")))
            end
        end
    end
end
