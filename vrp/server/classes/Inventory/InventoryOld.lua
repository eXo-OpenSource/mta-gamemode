...
-- *
-- ****************************************************************************
InventoryOld = inherit(Object)

function InventoryOld:constructor(owner, inventorySlots, itemData, classItems)
...
function InventoryOld:destructor()
	self.m_Items = nil
	self.m_Bag = nil
	InventoryManagerOld:getSingleton():deleteInventory(self.m_Owner)
end

function InventoryOld:syncClient()
...
function InventoryOld:loadItem(id)
...
function InventoryOld:useItem(itemId, bag, itemName, place)
...
function InventoryOld:useItemSecondary(itemId, bag, itemName, place)
	if self:getItemAmount(itemName) <= 0 then
		client:sendError(_("Inventar Fehler: Kein Item", client))
		return
	end
...
function InventoryOld:getItemID(bag, place)
	if self.m_Bag then
		return self.m_Bag[bag][place]
	end
end

function InventoryOld:getMaxItemAmount(item)
	return self.m_ItemData[item]["Item_Max"]
end

function InventoryOld:getLowEmptyPlace(bag)
...
function InventoryOld:getPlaces(bag)
...
function InventoryOld:getItemValueByBag(bag, place)
...
function InventoryOld:isPlaceEmpty(bag, place)
	local id = self:getItemID(bag, place)
...
function InventoryOld:setItemValueByBag(bag, place, value)
	if bag and place then
		local id = self:getItemID(bag, place)
		if id then
...
function InventoryOld:getItemPlacesByName(item)
	local placeTable = {}
...
function InventoryOld:removeItemFromPlace(bag, place, amount, value)
	local id = self:getItemID(bag, place)
...
function InventoryOld:getFreePlacesForItem(item)
	if self.m_Debug then
...
function InventoryOld:removeItem(item, amount, value)
	if self.m_Debug then
...
function InventoryOld:removeAllItem(item, value)
	if self.m_ItemData[item] then
		local bag = self.m_ItemData[item]["Tasche"]
		local places = self:getPlaces(bag)
		local id,itemName
...
function InventoryOld:getPlaceForItem(item, itemAmount)
	if self.m_ItemData[item] then
		local bag = self.m_ItemData[item]["Tasche"]
		local id
...
function InventoryOld:getItemAmount(item, inStack)
	if self.m_Debug then
...
function InventoryOld:throwItem(item, bag, id, place, name)
...
function InventoryOld:giveItem(item, amount, value)
	if self.m_Debug then
...
function InventoryOld:getItemWearLevelByBag(bag, place)
	if bag and place then
		local id = self:getItemID(bag, place)
		if id then
...
function InventoryOld:setItemWearLevelByBag(bag, place, wearLevel)
	if bag and place then
		local id = self:getItemID(bag, place)
		if id then
...
function InventoryOld:decreaseItemWearLevelByBag(bag, place)
	if bag and place then
		local id = self:getItemID(bag, place)
		if id then
...
function InventoryOld:decreaseItemWearLevel(itemName)
