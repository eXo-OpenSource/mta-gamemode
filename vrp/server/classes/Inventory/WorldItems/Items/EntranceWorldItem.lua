...
-- *
-- *  PROJECT:     vRoleplay
-- *  FILE:        server/classes/Inventory/WorldItems/Items/EntranceWorldItem.lua
-- *  PURPOSE:
-- *
-- ****************************************************************************
EntranceWorldItem = inherit(PlayerWorldItem)
EntranceWorldItem.Map = {}
--addRemoteEvents{"onDoorDataChange"}
--addEvent("onKeyPadSignal")

function EntranceWorldItem.onPlace(player, placingInfo, position, rotation)
	...
	EntranceWorldItem:new(placingInfo.itemData, player:getId(), player:getId(), DbElementType.Player, position, rotation, dim, int, true, "", {}, true, false)
end

function EntranceWorldItem:constructor(itemData, placedBy, elementId, elementType, position, rotation, dimension, interior, isPermanent, value, metadata, breakable, locked, databaseId)
    EntranceWorldItem.Map[self.m_Id] = self
--[[
