...
function DrawContestGUI:constructor()
	...
	self.m_Skribble = GUIGridSkribble:new(1, 1, 24, 14, self.m_Window)
	...
	for i, color in pairs(predefinedColors) do
		local colorButton = GUIGridRectangle:new(i, 15, 1, 1, Color[color], self.m_Window)
		...
		GUIGridEmptyRectangle:new(i, 15, 1, 1, 1, Color.Black, self.m_Window)
	end

	self.m_ChangeColor = GUIGridIconButton:new(13, 15, FontAwesomeSymbols.Brush, self.m_Window):setBarEnabled(false):setBackgroundColor(Color.Primary)
	self.m_ChangeColor.onLeftClick = bind(DrawContestGUI.changeColor, self)

	local erase = GUIGridIconButton:new(14, 15, FontAwesomeSymbols.Erase, self.m_Window):setBarEnabled(false):setBackgroundColor(Color.Primary)
	erase.onLeftClick = function() self.m_Skribble:setDrawColor(Color.White) end

	local clearDraw = GUIGridIconButton:new(15, 15, FontAwesomeSymbols.Trash, self.m_Window):setBarEnabled(false):setBackgroundColor(Color.Primary)
	...
	local slider = GUIGridSlider:new(16, 15, 5, 1, self.m_Window):setRange(10, 300)
	slider.onUpdate = function(size) self.m_Skribble:setDrawSize(size) end

	local save = GUIGridButton:new(22, 15, 3, 1, "Einsenden", self.m_Window)
	save.onLeftClick = function()
		QuestionBox:new("Möchtest du das Bild wirklich einsenden? Warnung: Du kannst nur ein einziges Bild für das Event einsenden!", function()
			triggerLatentServerEvent("onDrawContestSave", 5000, false, root, toJSON(self.m_Skribble:getSyncData()))
