...
function SniperShader:constructor( dur)
	...
	if ran == 1 then
		self.m_RandomStart = 0.4
	else 
		self.m_RandomStart = 0.6
	end
...
function SniperShader:update()
	if self.m_SniperShader and self.m_ScreenSource then
		...
		local prog = elap/dur
		local blur, center, sizefactor = interpolateBetween(0.6,self.m_RandomStart,1,0,0.5,1.3,prog,"OutBack")
		local rot = interpolateBetween(-3,0,0,0,3,0,prog*1.1,"CosineCurve")
