...
function GroupPropertyGUI:constructor( tObj ) 
	...
	self.m_LockButton = GUIButton:new(self.m_Width*0.25, self.m_Height*0.1, self.m_Width*0.5, self.m_Height*0.1, _"Auf-/Abschließen", tabManage):setBackgroundColor(Color.Orange)
	...
	GUIRectangle:new(self.m_Width*0.2, self.m_Height*0.25, self.m_Width*0.6, self.m_Height*0.01, tocolor(200,200,200,255),tabManage)
	self.m_MessageButton = GUIButton:new(self.m_Width*0.25, self.m_Height*0.32, self.m_Width*0.5, self.m_Height*0.1, _"Eingangsnachricht", tabManage):setBackgroundColor(Color.Orange)
	self.m_MessageButton.onLeftClick = bind(self.newMessageWindow,self)
	GUIRectangle:new(self.m_Width*0.2, self.m_Height*0.47, self.m_Width*0.6, self.m_Height*0.01, tocolor(200,200,200,255),tabManage)
	self.m_SellButton = GUIButton:new(self.m_Width*0.25, self.m_Height*0.55, self.m_Width*0.5, self.m_Height*0.1, _"Verkaufen", tabManage):setBackgroundColor(Color.Red)
	...
	self.m_KeyRemoveAllButton = GUIButton:new(self.m_Width*0.45, self.m_Height*0.52, self.m_Width*0.5, self.m_Height*0.08, _"Alle Abnehmen", tabAccess):setBackgroundColor(Color.Red)
	self.m_KeyRefreshButton = GUIButton:new(self.m_Width*0.45, self.m_Height*0.62, self.m_Width*0.5, self.m_Height*0.08, _"Aktualisieren", tabAccess):setBackgroundColor(Color.Orange)
	...
	self.m_KeyGrid = GUIGridList:new(self.m_Width*0.01, self.m_Height*0.32, self.m_Width*0.42, self.m_Height*0.38, tabAccess)
	...
	GUILabel:new(self.m_Width*0.1, self.m_Height*0.24, self.m_Width*0.65, self.m_Height*0.10, "Kaufpreis: "..self.m_PropertyTable.m_Price.."$", tabInfo):setFont(VRPFont(self.m_Height*0.1))
	GUILabel:new(self.m_Width*0.1, self.m_Height*0.36, self.m_Width*0.65, self.m_Height*0.10, "Verkaufpreis (System): "..math.floor(self.m_PropertyTable.m_Price*0.66).."$", tabInfo):setFont(VRPFont(self.m_Height*0.1))
	GUILabel:new(self.m_Width*0.1, self.m_Height*0.48, self.m_Width*0.65, self.m_Height*0.10, "Lage: "..getZoneName(tObj.m_CamMatrix[1],tObj.m_CamMatrix[2],tObj.m_CamMatrix[3]), tabInfo):setFont(VRPFont(self.m_Height*0.1))
...
function GroupPropertyGUI:setGroupDoorState( b ) 
	self.m_Open = b
	if b == 0 then 
		--self.m_KeyBackground = GUIRectangle:new( self.m_Width*0.7+(self.m_Height*0.1), self.m_Height*0.1, self.m_Height*0.1, self.m_Height*0.1,Color.Orange,self.m_TabManage)
		self.m_KeyImage = GUIImage:new(self.m_Width*0.7+(self.m_Height*0.1), self.m_Height*0.1, self.m_Height*0.1, self.m_Height*0.1,"files/images/Other/KeyIcon.png", self.m_TabManage)
		self.m_KeyImageDrawn = true
	else 
		if self.m_KeyImageDrawn then 
			self.m_KeyImageDrawn = false
			self.m_KeyImage:delete()
			--self.m_KeyBackground:delete()
		end
	end
end
