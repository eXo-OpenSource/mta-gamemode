...
function VehiclePerformanceGUI:fillAllTabs(  )
	...
	for i = 1, #self.m_Tabs do 
		...
		if i == 1 then
			...
			self.m_ScrollAreas[i] = GUIGridScrollableArea:new(1, 1, 12, 12, 16, documentHeight+4, true, false, tab)
			self.m_Offset = 1
			for category, handling in pairs(VEHICLE_TUNINGKIT_CATEGORIES) do 
	...
	for i = 1, #handling do 
		...
		if VEHICLE_TUNINGKIT_DESCRIPTION[propName] then
			...
			elseif propName == "driveType" then 
				...
				table.insert(self.m_LabelDescription[propName], GUIGridLabel:new(5, self.m_Offset+1, 2, 1, "All", scroll))
				table.insert(self.m_LabelDescription[propName], GUIGridLabel:new(10, self.m_Offset+1, 2, 1, "Vorder", scroll))
				self.m_SliderBars[propName][#self.m_SliderBars[propName]]:setRange(0, 100):setEnabled(self.m_Modify)
