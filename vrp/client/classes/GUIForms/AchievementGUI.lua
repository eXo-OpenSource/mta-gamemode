...
function AchievementGUI:constructor(money)
	...
	self.m_Window = GUIWindow:new(0, 0, self.m_Width, self.m_Height, _"Achievments", true, true, self)
	...
	GUILabel:new(self.m_Width*0.45, self.m_Height*0.07, self.m_Width*0.55, self.m_Height*0.08, _"Achievments", self.m_Window)
	GUILabel:new(self.m_Width*0.45, self.m_Height*0.15, self.m_Width*0.55, self.m_Height*0.05, _"Auf eXo-Reallife gibt es viele Achievments mit verschiedenen Belohnungen die gefunden bzw. erledigt werden können. Einige davon sind vesteckt, und tauchen erst in der Liste auf wenn du diese findest!", self.m_Window):setMultiline(true)
...
function AchievementGUI:onShow()
	...
	self.m_PlayerAchievments = localPlayer:getAchievements()
	self:loadGridList()
end
...
function AchievementGUI:onAchievmentClick(id)
	local data = Achievement:getSingleton():getAchievements()[id]
	if self.m_PlayerAchievments[id] or data["hidden"] == 0 then
