...
function FriskGUI:constructor(player, weapons, drugs)
	...
	local tabDrugs = self.m_TabPanel:addTab(_("Illegales"))
	...
	self.m_DrugList:addColumn("Illegales", .6)
	self.m_DrugList:addColumn("Gewicht / Anzahl", .4)
	...
	for drug, amount in pairs(drugs) do
		self.m_DrugList:addItem(drug, ("%d"):format(amount))
	end
end
