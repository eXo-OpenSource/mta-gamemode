...
function ItemEntrance:constructor( id, pos )
	...
	self.m_Info = GUIGridLabel:new(1, 4, 6, 1, "Gib hier die ID des anderen Eingangs, falls Interior > 0", self.m_Window):setFont(VRPFont(16, Fonts.EkMukta))
	...
	self.m_AcceptButton = GUIGridButton:new(1, 5, 4, 1, FontAwesomeSymbols.Close, self.m_Window):setFont(FontAwesome(20)):setColor(tocolor(140, 0, 0, 255)):setBarEnabled(false):setBackgroundColor(tocolor(90, 90, 90, 255))
	self.m_AcceptButton.onLeftClick = bind(self.closeForm, self)
	self.m_DeclineButton = GUIGridButton:new(6, 5, 4, 1, FontAwesomeSymbols.Accept, self.m_Window):setFont(FontAwesome(20)):setColor(tocolor(0, 140, 0, 255)):setBarEnabled(false):setBackgroundColor(tocolor(90, 90, 90, 255))
...
function ItemEntrance:submitForm() 
	...
	triggerServerEvent("onEntranceDataChange", localPlayer, addKeyPad, removeKeyPad, interior, posX, posY, posZ, entranceLink)
	delete(self)
end