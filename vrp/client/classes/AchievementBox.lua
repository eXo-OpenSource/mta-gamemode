...
function AchievementBox:constructor (text, xp)
    if AchievementBox.active then
        table.insert(AchievementBox.queue, {id, text})
        return
    end
    ...
    dxSetRenderTarget(self.ms_renderTarget, true)
        ...
        dxDrawText(("%s XP - %s"):format(self.m_XP, self.m_Text), 80, 35, 360, self.ms_endSize.y - 15, Color.White, 1, self.m_Font, "center", "center")
