...
addCommandHandler("texnames", function()
	-- Version check
	...
	if active then
		removeEventHandler("onClientRender", root, drawTextureNames)
		removeEventHandler("onClientKeyClick", resourceRoot, onKeyPress)
		active = false
	else
		-- Create shader
		...
		if not myShader then
			...
			addEventHandler("onClientRender", root, drawTextureNames)
			addEventHandler("onClientKeyClick", resourceRoot, onKeyPress)
		end
	end
end)
