	...
	for index,value in ipairs(inTable) do
		if inTable[index].enabled then
			...
			if dist <= distFade then 
				...
				outTable[w].attachedTo = value.attachedTo	
				outTable[w].attachOffset = value.attachOffset	
				outTable[w].pos = value.pos		
				if outTable[w].attachedTo and isElement(outTable[w].attachedTo) then
					local pos = outTable[w].attachedTo.matrix:transformPosition(outTable[w].attachOffset)
					outTable[w].pos = {pos.x, pos.y, pos.z}
					value.pos = {pos.x, pos.y, pos.z}
				end
			end
		end
	end
