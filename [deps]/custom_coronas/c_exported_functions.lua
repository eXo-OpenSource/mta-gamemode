...
function attachCoronaTo(w, ele)
	if not isElement(w) then 
		return false
	end
	local SHCelementID = tonumber(getElementID(w))
	if coronaTable.inputCoronas[SHCelementID] then 
		coronaTable.inputCoronas[SHCelementID].attachedTo = ele
		coronaTable.inputCoronas[SHCelementID].attachOffset = Vector3(0, 0, 0)
		coronaTable.isInValChanged = true
		return true
	else
		outputDebugString('attachCoronaTo fail!')
		return false
	end
end

function setAttachedOffsets(w, x, y, z)
	if not isElement(w) then 
		return false
	end
	local SHCelementID = tonumber(getElementID(w))
	if coronaTable.inputCoronas[SHCelementID] and coronaTable.inputCoronas[SHCelementID].attachedTo then 
		coronaTable.inputCoronas[SHCelementID].attachOffset = Vector3(x, y, z)
		coronaTable.isInValChanged = true
		return true
	else
		outputDebugString('setAttachedOffsets fail!')
		return false
	end
end
