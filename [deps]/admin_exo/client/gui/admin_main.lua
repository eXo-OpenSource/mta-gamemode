...
function aAdminMenu ()
	if ( aAdminForm == nil ) then
		...
		guiSetEnabled(aTab1.Spectate, false)
		...
		if ( getElementParent ( source ) == aTab1.Tab ) then
			...
			elseif ( getElementType ( source ) == "gui-button" )  then
				...
				if ( guiGridListGetSelectedItem ( aTab1.PlayerList ) == -1 ) then
					...
					elseif ( source == aTab1.Spectate ) then outputChatBox("Disabled function")
...
function FormatDate(format, escaper, timestamp)
	...
	local datetime = { d = ("%02d"):format(time.monthday), h = ("%02d"):format(time.hour), i = ("%02d"):format(time.minute), m = ("%02d"):format(time.month), s = ("%02d"):format(time.second), w = gWeekDays[time.weekday+1]:sub(1, 2), W = gWeekDays[time.weekday+1], y = tostring(time.year):sub(-2, -1), Y = time.year }
